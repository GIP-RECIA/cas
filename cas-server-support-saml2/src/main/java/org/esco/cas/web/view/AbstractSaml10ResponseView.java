...
public abstract class AbstractSaml10ResponseView extends AbstractCasView {
	...
	@NotNull
	private String encoding = AbstractSaml10ResponseView.DEFAULT_ENCODING;
	...
	protected void renderMergedOutputModel(
		...
		try {
			final Response samlResponse = this.newSamlObject(Response.class);
	...
	protected final <T extends SAMLObject> T newSamlObject(final Class<T> objectType) {
		final QName qName;
		try {
			final Field f = objectType.getField(AbstractSaml10ResponseView.DEFAULT_ELEMENT_NAME_FIELD);
			qName = (QName) f.get(null);
		} catch (final NoSuchFieldException e) {
			throw new IllegalStateException("Cannot find field " + objectType.getName() + "." + AbstractSaml10ResponseView.DEFAULT_ELEMENT_NAME_FIELD);
		} catch (final IllegalAccessException e) {
			throw new IllegalStateException("Cannot access field " + objectType.getName() + "." + AbstractSaml10ResponseView.DEFAULT_ELEMENT_NAME_FIELD);
		}
		final SAMLObjectBuilder<T> builder = (SAMLObjectBuilder<T>) org.opensaml.xml.Configuration.getBuilderFactory().getBuilder(qName);
		if (builder == null) {
	...
	protected final Status newStatus(final QName codeValue, final String statusMessage) {
		final Status status = this.newSamlObject(Status.class);
		final StatusCode code = this.newSamlObject(StatusCode.class);
		code.setValue(codeValue);
		status.setStatusCode(code);
		if (statusMessage != null) {
			final StatusMessage message = this.newSamlObject(StatusMessage.class);
