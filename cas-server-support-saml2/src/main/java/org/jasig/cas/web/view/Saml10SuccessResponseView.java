...
public final class Saml10SuccessResponseView extends AbstractSaml10ResponseView {
	...
	@NotNull
	private String rememberMeAttributeName = Saml10SuccessResponseView.REMEMBER_ME_ATTRIBUTE_NAME;

	@Override
	protected void prepareResponse(final Response response, final Map<String, Object> model) {
		final Authentication authentication = this.getAssertionFrom(model).getChainedAuthentications().get(0);
		final DateTime issuedAt = response.getIssueInstant();
		final Service service = this.getAssertionFrom(model).getService();
		final boolean isRemembered = (
				...
				&& !this.getAssertionFrom(model).isFromNewLogin());

		// Build up the SAML assertion containing AuthenticationStatement and AttributeStatement
		final Assertion assertion = this.newSamlObject(Assertion.class);
	...
	private Conditions newConditions(final DateTime issuedAt, final String serviceId) {
		final Conditions conditions = this.newSamlObject(Conditions.class);
		conditions.setNotBefore(issuedAt);
		conditions.setNotOnOrAfter(issuedAt.plus(this.issueLength));
		final AudienceRestrictionCondition audienceRestriction = this.newSamlObject(AudienceRestrictionCondition.class);
		final Audience audience = this.newSamlObject(Audience.class);
	...
	private Subject newSubject(final String identifier) {
		final SubjectConfirmation confirmation = this.newSamlObject(SubjectConfirmation.class);
		final ConfirmationMethod method = this.newSamlObject(ConfirmationMethod.class);
		method.setConfirmationMethod(Saml10SuccessResponseView.CONFIRMATION_METHOD);
		confirmation.getConfirmationMethods().add(method);
		final NameIdentifier nameIdentifier = this.newSamlObject(NameIdentifier.class);
		nameIdentifier.setNameIdentifier(identifier);
		final Subject subject = this.newSamlObject(Subject.class);
	...
	private AttributeStatement newAttributeStatement(
		...
		final AttributeStatement attrStatement = this.newSamlObject(AttributeStatement.class);
		...
		for (final Entry<String, Object> e : attributes.entrySet()) {
			if ((e.getValue() instanceof Collection<?>) && ((Collection<?>) e.getValue()).isEmpty()) {
				...
				this.log.info("Skipping attribute {} because it does not have any values.", e.getKey());
				continue;
			}
			final Attribute attribute = this.newSamlObject(Attribute.class);
			attribute.setAttributeName(e.getKey());
			attribute.setAttributeNamespace(Saml10SuccessResponseView.NAMESPACE);
		...
		if (isRemembered) {
			...
			attribute.setAttributeNamespace(Saml10SuccessResponseView.NAMESPACE);
			attribute.getAttributeValues().add(this.newAttributeValue(Saml10SuccessResponseView.REMEMBER_ME_ATTRIBUTE_VALUE));
			attrStatement.getAttributes().add(attribute);
		}
